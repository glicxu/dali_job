"""create job cache and user saved job storage
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(["jobs_cache_id"], ["jobs_cache.id"], ondelete="CASCADE"),
...
def downgrade() -> None:
    op.drop_index("ix_user_saved_jobs_workspace_id", table_name="user_saved_jobs")
    op.drop_index("ix_user_saved_jobs_user_id", table_name="user_saved_jobs")
    op.drop_index("ix_user_saved_jobs_jobs_cache_id", table_name="user_saved_jobs")
